...
export const clientLoader = async ({ request }: Route.ClientLoaderArgs) => {
  ...
  if (qs.search) {
    query = query.ilike("code", "%" + qs.search.replace(" ", "%") + "%");
  }
...
const Major: React.FC<Route.ComponentProps> = ({ loaderData }) => {
  ...
  const [showEdit, setShowEdit] = useState<Tables<"room">>();
  const [showDelete, setShowDelete] = useState<Tables<"room">>();
  ...
  return (
    <div className="max-w-xl mx-auto">
      <TopSection title="Ruangan" onAdd={() => setShowAdd(true)} />
